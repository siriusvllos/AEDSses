...
private static void swap(Pokemon []pokedex, int i, int menor) {
        Pokemon aux = pokedex[i];
        pokedex[i] = pokedex[menor];
        pokedex[menor] = aux;
    }

    private static void heapSort(Pokemon []pokedex, int tam, int[] comp, int[] mov) {
        for (int i = tam / 2 - 1 ; i >= 0 ; i--) {
            heap(pokedex, tam, i, comp, mov);
        }
        ...
        for (int i = tam - 1 ; i >= 0 ; i--) {
            ...
            heap(pokedex, i, 0, comp, mov);
        }
    }
    private static void heap(Pokemon []findPokemon, int n, int i, int[] comp, int[] mov) {
        int largest = i;
        int left = 2 * i + 1;
        int right = 2 * i + 2;
        ...
        if (left < n) {
            comp[0]++;
            if ((findPokemon[left].getHeight() > findPokemon[largest].getHeight()) ||
                ((findPokemon[left].getHeight() == findPokemon[largest].getHeight()) && (findPokemon[left].getName().compareTo(findPokemon[largest].getName())) > 0)) {
        ...
        if (right < n) {
            comp[0]++;
            if ((findPokemon[right].getHeight() > findPokemon[largest].getHeight()) ||
                ((findPokemon[right].getHeight() == findPokemon[largest].getHeight()) && (findPokemon[right].getName().compareTo(findPokemon[largest].getName())) > 0)) {
                largest = right;
            }
        }
        ...
        if (largest != i) {
            ...
            heap(findPokemon, n, largest, comp, mov);
        }
    ...
    public static void main(String[] args) {
        ...
        heapSort(pokedex, tam, comp, mov);

        for (int j = 0 ; j < tam ; j++) {
            System.out.println(pokedex[j].imprimir());
        }

        long end = System.nanoTime();

        double executionTime = (end - startTime + 5);

        String conteudo = "835625" + "\t" + comp[0] + "\t" + mov[0] + '\t' + executionTime + "ms";

        try (BufferedWriter writer = new BufferedWriter(new FileWriter("835625_heapsort.txt"))) {
