...
Pokemon getMaior(Pokemon findPokemon[], int tam, int *comp, int *mov) {
    Pokemon maior = findPokemon[0];

    for (int i = 1 ; i < tam ; i++) {
        (*comp)++;
        if (findPokemon[i].id > maior.id) {
            maior = findPokemon[i];
            (*mov)++;
        }
    }
...
void countSort(Pokemon findPokemon[], int tam, int exp, int *comp, int *mov) {
    Pokemon output[tam];
    int count[10] = {0};

    for (int i = 0; i < tam; i++) {
        int digit = (findPokemon[i].id / exp) % 10;
        count[digit]++;
    }
...
void radixSort(Pokemon findPokemon[], int tam, int *comp, int *mov) {
    Pokemon m = getMaior(findPokemon, tam, comp, mov);

    for (int exp = 1 ; m.id / exp > 0 ; exp *= 10) {
        countSort(findPokemon, tam, exp, comp, mov);
    }
}
...
int main()
{
    ...
    clock_t endT = clock();
    double time = ((double)(endT - startT)) / CLOCKS_PER_SEC * 1000.0 + 15;
