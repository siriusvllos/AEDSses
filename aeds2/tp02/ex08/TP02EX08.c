...
void shellSort(Pokemon lista[], int tam, int *comp, int *mov)
{
    for (int gap = tam / 2; gap > 0; gap /= 2)
    {
        for (int i = gap; i < tam; i++)
        {
            ...
            while (j >= gap)
            {
                bool weight = lista[j - gap].weight > key.weight;
                bool weightEquals = lista[j - gap].weight == key.weight;
                int name = strcmp(lista[j - gap].name, key.name);
