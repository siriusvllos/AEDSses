...
private static void swap(Pokemon []pokedex, int i, int menor) {
    ...
    private static void selectionSort(Pokemon []pokedex, int tam, int[] comp, int[] mov) {
            for (int i = 1 ; i < tam ; i++) {
                Pokemon key = pokedex[i];
                int j = i - 1;

                while (j >= 0) {
                    int date = key.getCaptureDate().compareTo(pokedex[j].getCaptureDate());
                    int name = key.getName().compareTo(pokedex[j].getName());
                    comp[0]++;
    ...
    public static void main(String[] args) {
        ...
        selectionSort(pokedex, tam, comp, mov);
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("835625_insertion.txt"))) {
