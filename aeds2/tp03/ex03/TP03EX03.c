...
Pokemon *acharPokemon(Pokemon pokedex[], int n, int id)
{
    for (int i = 0; i < n; i++)
    {
        if (pokedex[i].id == id)
...
int main()
{
    ...
    Pilha pPilha = inicializarPilha(100);
    ...
    {
        int id = atoi(inputId);
        Pokemon *p = acharPokemon(pokedex, n, id);
        if (p != NULL)
        {
            inserir(&pPilha, p);
        }
    ...
    for (int i = 0; i < nc; i++)
    {
        ...
        else if (strcmp(cms, "R") == 0)
        {
            Pokemon *p = remover(&pPilha);
            if (p != NULL)
            {
                printf("(R) %s\n", p->name);
            }
