...
// Oi, aqui é o Sirius
// Basicamente oq eu estou tentando fazer é a ordenação binaria igual a questão de java, mas em C
...
int main()
{
    ...
    for (int i = 0 ; i < j ; i++) {
        ...
        for (int k = i + 1 ; k < j ; k++) {
            comp++;
            if (strcmp(pokedex[menor].name, pokedex[k].name) > 0) {
                menor = k;
            }
        }
        if (menor != i) {
    ...
    while(strcmp(name, "FIM") != 0) {
        ...
        while (inicio <= fim) {
            ...
            int cmp = strcmp(name, pokedex[meio].name);
    ...
    FILE *arquivo = fopen("835625_binaria.txt", "w");
    if (arquivo == NULL) {
        printf("Erro\n");
        return 1;
    }
